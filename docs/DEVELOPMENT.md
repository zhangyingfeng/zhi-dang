...
| 本地服务端口 | `4317` | `4318`——两个 edition 的 `.app` 可以同时打开，图标和端口都不会撞（图标生成见 `scripts/generate-login-icon.py`，从共享的 `assets/app-icon-source.png` 反色生成；共享主图本身按苹果官方 macOS 图标模板规格重做过，见 `scripts/apply-apple-icon-spec.py`） |
