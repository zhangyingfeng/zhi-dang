...
- `ContentSource` interface (`src/source/types.ts`) is the one seam between "where content comes from" and everything else. `LoginContentSource` wraps the existing webview-relay pagination unchanged; `KeyContentSource` hits `user/contents` (listing) and `user/content_detail` (full text).
- Two thin entrypoints (`src/index.login.ts`, `src/index.key.ts`) share `src/server.ts`'s `createServer()`. Rust/Tauri command surface is split via `#[cfg(not(feature = "key"))]` vs `#[cfg(feature = "key")]` rather than a positive "login" feature, since Tauri CLI's `-f/--features` can only add features, never disable the default. The key edition stores its Access Secret in the macOS Keychain (`keyring` crate); it has no login window and no session relay at all.
