...
type ApiPage = { data: any[]; paging: { is_end: boolean; next?: string; totals?: number } };
...
export class ZhihuClient {
  ...
  async list(kind:ContentKind,token:string,onCount?:(n:number)=>void){
    const output:ZhihuItem[]=[]; const seen=new Set<string>(); let completed=false; let expectedTotal:number|undefined; let received=0; let duplicates=0;
    let url=kind==="answer"?`https://www.zhihu.com/api/v4/members/${token}/answers?include=data[*].content,excerpt,created_time,updated_time,voteup_count,favlists_count,comment_count,thumbnail,question.title,question.id&limit=20&offset=0&sort_by=created`:`https://www.zhihu.com/api/v4/members/${token}/articles?include=data[*].content,excerpt,created,updated,voteup_count,favlists_count,comment_count,title,url,image_url&limit=20&offset=0&sort_by=created`;
    for(let guard=0;guard<1000;guard++){
      ...
      for(const raw of page.data){ const item=this.normalize(kind,raw); const key=`${kind}:${item.id}`; if(!seen.has(key)){seen.add(key);output.push(item);}else duplicates++; }
      ...
      url=page.paging.next; await sleep(800);
    }
    ...
    return {items:output,report:{kind,reportedTotal:expectedTotal??null,received,unique:output.length,duplicates,warning}} satisfies ListingResult;
  }
  private normalize(kind:ContentKind,x:any):ZhihuItem {
