...
Adds a second, independently-buildable **official-API edition** alongside the existing **direct-connect edition** — same archive engine, UI shell, and version number, differing only in how content is fetched. This is what makes an eventual App Store submission possible: the direct edition replays an authenticated web session (blocked by Guideline 5.2.2), the official edition authenticates via a Zhihu-issued Access Secret against the official Data Open Platform instead.

- `ContentSource` interface (`src/source/types.ts`) is the one seam between "where content comes from" and everything else. `DirectContentSource` wraps the existing webview-relay pagination unchanged; `OfficialApiContentSource` hits `user/contents` (listing) and `user/content_detail` (full text).
- Two thin entrypoints (`src/index.direct.ts`, `src/index.official.ts`) share `src/server.ts`'s `createServer()`. Rust/Tauri command surface is split via `#[cfg(not(feature = "appstore"))]` vs `#[cfg(feature = "appstore")]` rather than a positive "direct" feature, since Tauri CLI's `-f/--features` can only add features, never disable the default. The official edition stores its Access Secret in the macOS Keychain (`keyring` crate); it has no login window and no session relay at all.
- **Fixed a real bug found while building this**: `Exporter.export` used to `break` the export loop on quota exhaustion. If quota ran out partway through a newest-first sorted list, any already-successful item positioned after that point never got the chance to be re-affirmed into the manifest on that run, and was silently dropped from `index.json` — the `.md` file stayed on disk untouched, only the bookkeeping lost it, and repeated attempts against an already-exhausted quota made it worse each time. It now `continue`s: the loop keeps scanning every remaining item (without spending quota on it) so already-done/skipped entries are always preserved. See `test/exporter-quota.test.ts`.
- Access Secret is validated against a quota-cost-free lookup before it's ever saved or treated as "logged in." "开始导出" is disabled outright once today's quota is confirmed exhausted (with a toast at login and a native dialog if it runs out mid-run), instead of only surfacing a bad secret or an exhausted quota on the first failed export.
- The official API has no endpoint that returns the account holder's own name — every relevant endpoint was checked; it looks deliberate, not a gap. An optional one-time profile-URL paste (parsed locally, never sent anywhere) substitutes for the welcome text and default folder name the direct edition already gets from `url_token`.
- Zhihu OAuth was evaluated (it would provide a real display name/avatar) and explicitly rejected, not deferred: the docs treat `app_key` as a confidential-client secret with no PKCE or public-client story, which doesn't hold up in an open-source desktop app, and a token-relay server to work around that reintroduces exactly the hosting responsibility 知档 is scoped to avoid. Access Secret is the permanent, only auth path for this edition. See `docs/DEVELOPMENT.md`.
...
Mac App Store signing identity, App Sandbox entitlements, notarization, and App Store Connect submission are **not** part of this release — see ROADMAP.md's 1.3/1.4. This release ships the official edition as a second downloadable build alongside the direct edition, same as today's distribution channel, while that work is pending.
