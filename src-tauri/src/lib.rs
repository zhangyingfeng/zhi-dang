...
mod login {
  ...
  #[tauri::command]
  pub fn zhihu_fetch_result(state: tauri::State<'_, PendingFetches>, id: u64, status: u16, body: String) {
...
mod key {
  ...
  #[tauri::command]
  pub fn save_access_secret(secret: String) -> Result<(), String> {
    let trimmed = secret.trim();
    if trimmed.is_empty() {
      return Err("Access Secret 不能为空".to_string());
    }
    entry()?.set_password(trimmed).map_err(|e| e.to_string())
  }

  #[tauri::command]
  pub fn has_access_secret() -> bool {
  ...
  #[tauri::command]
  pub fn clear_access_secret() -> Result<(), String> {
