...
import type { ExportOptions, ListingReport, TaskEvent, ZhihuItem } from "./types.js";
...
export class Exporter {
  ...
  async export(items:ZhihuItem[],listingReports:ListingReport[],opts:ExportOptions,onEvent:(e:TaskEvent)=>void){
    ...
    for(let i=0;i<items.length;i++){
      const item=items[i]; onEvent({type:"start",id:item.id});
      try{
