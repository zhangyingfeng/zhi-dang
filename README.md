...
**知档的产品边界**（这些不是没做完，是刻意不做）

| 不属于知档 | 为什么 |
|---|---|
| 从归档生成网站，以及网站托管、域名、DNS | 发布是另一件事，混进"归档"这层会让它变得不稳定，交给独立项目做 |
| 内容编辑、草稿、版本历史 | 知档存的是已发布内容的忠实副本，不是创作工具 |
| AI 分类、摘要、检索、个人写作模型 | 这类功能依赖持续变化的模型和算法，知档想保持的是"很多年后打开还能用"的稳定数据层，两种目标放在一起会互相拖累 |
| 云同步、多设备账号体系 | 一旦加云同步，"内容不经过任何服务器"这条前面所有的承诺就不成立了 |

这是独立开发的非官方项目，与知乎不存在隶属、合作、授权或认可关系。使用者有责任遵守适用法律、著作权规则和平台条款。
产品范围划分见 [ROADMAP.md](docs/ROADMAP.md#范围控制)，开发计划见 [ROADMAP.md](docs/ROADMAP.md)。
