...
导出完成后会显示汇总信息，同时"开始导出"按钮会变成"在访达中显示"（同一个按钮换了文字和功能，不是新出现一个按钮），点一下就能在 Finder 里定位到导出目录。如果这时候改了保存位置准备导出到别的地方，这个按钮会自动变回"开始导出"。
...
点这个已经变成"在访达中显示"的按钮，会在 Finder 里直接定位到导出目录：
