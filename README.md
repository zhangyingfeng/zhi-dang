...
知档现在有两个版本（edition），归档格式和隐私原则完全一致，只是内容获取方式不同：

- **直连版**（默认下载的版本）：本人登录后复用会话直连知乎网页接口，跟上面的边界说明完全对应。
- **官方 API 版**：改用知乎官方数据开放平台的 Access Secret 鉴权，不需要登录窗口，但每天有官方额度限制（100 次，未实名 10 次）；因为访问方式是知乎官方授权的，这个版本具备上架 App Store 的条件。目前只能从源码自行构建，还没有作为独立安装包发布。

两版怎么选、代码里怎么区分，见[开发者文档](docs/DEVELOPMENT.md)。
